...
namespace Specpen.Web.Services;
...
public sealed class CompareSelectionState
{
    ...
    private async Task<bool> HydrateCoreAsync()
    {
        try
        {
            ...
            var serialized = await _jsRuntime.InvokeAsync<string?>("specpenCompareSelection.load");
            IsHydrated = true;
    ...
    private async Task PersistAsync()
    {
        ...
        try
        {
            ...
            await _jsRuntime.InvokeVoidAsync("specpenCompareSelection.save", serialized);
        }
        catch (InvalidOperationException exception)
        {
